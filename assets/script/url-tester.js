$(function(){
	...
	function setURL(ambience, url){
		return _settings[ambience] = url;
	}

	function getURL(ambience){
		return _settings.url[ambience];
	}
	...
	function getQueryData(){
		...
		if(qs) {
			...
			for (var i = 0; i < qs.length; i++){
				var item = qs[i].split('=');
				if (item[0] === 'ambience' || item[0] === 'protocol'){
	...
	function generateURL(){
		...
		var ambience = getData('ambience');
		var url = [];
		var prettyText = [];

		url.push(protocol + '://' + getURL(ambience));
		url.push('?' + serializeParams().join('&'));

		prettyText.push( wrapTag(protocol, _settings.className.urlPart) + wrapTag('://', _settings.className.separator) + wrapTag( getURL(ambience), _settings.className.urlPart) );
	...
	var _settings = {
		...
		el: {
			$controlUI:			$('.controls').find('li.button'), // buttons for protocol, ambience ec
			...
			$birthdayCheckbox:  $('input#birthday')
		}, 
		...
		url: {
			posh:		'chez-carmine.com',
			rustic:		'western-heart.com',
			relaxed:	'backyard-grill.com'
		}
	};
	...
	_settings.el.$modalChooseBtn.click(function(event){
		event = _vent.getE(event).stopPropagation();
	...
	// On control button click (protocol, ambience, birthday)
	_settings.el.$controlUI.bind('click mouseover mouseout', function(e){
		...
		switch (e.type) {
			...
			case 'mouseout':
				updateUI($log, getURL( getData('ambience') ));
				break;

			default: 
				highlightSelected(this);
				setData(key, value, key === 'ambience' ? 'url' : '');
	...
	$('.url-options a').click(function(e){
	...
	_settings.el.$birthdayCheckbox.change(function(){
		if ( $(this).is(':checked') ) {
			setData('birthday', 'true', 'params');
		} else {
			deleteData('birthday', 'params');
		}
		updateUI( _settings.el.$output, generateURL() );
	});
